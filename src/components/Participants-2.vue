<template>
  ...
  <h2 class="font-display text-4xl mb-0 font-medium w-full flex flex-row">Featured Participants</h2>

  <div class="w-full py-8 px-0 flex flex-row">
   <swiper
            :options="sliderOptions"
            class='px-0'
          >
            <swiper-slide class="participant_slide"
                v-for="(organiser, indx) in organisers"
                :key="indx"
                :index="indx"
                >

              <div class="organiser flex flex-col shadow-lg rounded-lg p-4 bg-white" style="height: auto; width: 90% !important; margin: 0 auto !important; background-repeat: no-repeat">
                <div class="flex flex-row justify-center items-center mb-0 h-40 w-full">
                <img :src='organiser.user.avatar' class="w-1/4 h-auto rounded-lg" />
                <div class="w-3/4 pl-6">
                   <h4 class="font-body text-3xl flex items-center h-full content-center w-full mb-2 font-bold text-left  leading-none">{{organiser.name}}</h4>
                  <p class="text-body text-left">{{organiser.byline}}</p>
                  </div>
                 </div>
                   <p class="text-body text-left">{{organiser.bio}}</p>
